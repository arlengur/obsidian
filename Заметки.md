...
Проц:
- Двухъядерный ARM-процессор на 1.3–1.6 ГГц справляется с VPN, блокировкой рекламы и одновременной раздачей Wi-Fi на десятки устройств. 
	- Бюджетный MT7981 он же Filogic 820.
	- Флагманский MT7986 он же Filogic 830.
- Одноядерные MIPS-чипы с частотой до 600 МГц уже не подходят для сложных задач: они перегружаются даже при среднем трафике.

Чипсет:
- Для OpenWrt чаще всего рекомендуют чипсеты Qualcomm Atheros и MediaTek.
- Чипсеты от Broadcom часто вызывают проблемы: нестабильный Wi-Fi, отсутствие поддержки 5 ГГц, ограниченные или устаревшие драйверы. Иногда беспроводной модуль вообще не работает после прошивки.

Оперативная и флеш-память (RAM и ROM):
Для простой работы с OpenWrt минимально желательно:
- 128 МБ оперативной памяти — для базовых задач;
- 16 МБ встроенной памяти (ROM) — для прошивки и хранения настроек.

Оптимальный вариант для уверенной работы:
- 256 МБ и больше RAM — обеспечивает стабильность даже под нагрузкой, позволяет запускать несколько служб одновременно;
- 32 или 64 МБ ROM (или eMMC / NAND-память) — дает запас под расширенные прошивки, дополнительные модули и настройки.

Ethernet-порты: гигабитные, а не 100 Мбит/с.

USB-порт: пригодится, если вы планируете подключать внешний диск, 4G/5G модем или использовать роутер как сервер.

Wi-Fi: для 2025 года минимальный стандарт — Wi-Fi 5 (802.11ac), а лучше — Wi-Fi 6 (802.11ax) или Wi-Fi 7 (802.11be), особенно если у вас современные устройства или высокая нагрузка.

---

Amnezia mirror:
https://storage.googleapis.com/kldscp/amnezia.org

Amnezia error:
ErrorCode: 1200. QFile error: The file could not be opened решается перезапуском клиента

Модем на iPhone (hotspot)
https://www.eldorado.ru/blog/publications/kak-vernut-rezhim-modema-na-iphone-16-i-drugikh-modelyakh-43546
