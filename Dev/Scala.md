...
# Иерархия типов (Scala type system)
...
Any - супер тип
AnyVal - примитивные типы данных
AnyRef - сылочные типы данных
...
Null - подтип сылочных типов данных (нужен для обратной совместимости)
- один единственный экземпляр
- null - литерал
...
Функции применяемые к элементам List перебирают всю коллекцию, а в LazyList по одному элементу.
```scala
val list = List(1, 2, 3, 4, 5, 6)  
list.map(e => {println("!"); e}).map(e => {println("!!"); e}).head
...
m.contains("q")
val m1 = m + ("five" -> "fifth") // добавление элемента создает map
...
# Implicits
## 3 группы implicit
- преобразования (классы)
- параметры
- условные

Пример преобразования:
```scala
implicit def foo(str: String): Int = ??? // преобразование
// или
...
Пример условного:
```scala
// метод работает когда есть неявное преобразование 
