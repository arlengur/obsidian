...
Unit - тип который говорит о том что метод совершает сайд эффект (метод меняет внешний контекст по отношению к своему телу). Аналог void.
...
Объекты лениво инициализируются то есть только в момент обращения к объекту и являются синглетонами.
...
## Чистые функции
Pure functions - функции, которые для одних и тех же аргументов вычисляют одни и те же результаты и не делают никаких сайд эффектов.

f: A => B - по этой функции не понятно делает она сайд эффекты или нет 

def sum(x: Int, y: Int): Int = x + y - чистая функция

Позволяют делать кеширование и код становится более понятным.

## Referential transparency
Ссылочно прозрачная функция - все что делает функция, представлено ее возвращаемым
значением, согласно типа этого значения.
