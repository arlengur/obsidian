...
Postrgres когда читаем несколькими потребителями, то есть конструкция select update skip locked на уровне транзакции, позволит делать это без дублей.

В Postrgres запрос 
- explain - план запроса
- explain analize - выполняет план запроса и описывает все вызовы
...
Ускоряют поиск данных в большой таблице.

Индекс - карта доступа к элементам (древовидная структура)

B-tree индекс — это сбалансированная древовидная структура данных, используемая в СУБД (PostgreSQL, MySQL, Oracle) для ускорения поиска, сортировки и выборки диапазонов данных за логарифмическое время O(logN). Он хранит ключи отсортированными, обеспечивая эффективный доступ к диску. B-tree индексы автоматически поддерживаются при обновлении данных. Хорошо работает для операций `> < ==`

Хэш-индекс — это структура данных в базе данных, использующая хэш-функцию для быстрого поиска строк по точному совпадению (оператор =). Он преобразует значения столбца в числовые хэш-коды, обеспечивая время поиска O(1). Хэш-индексы эффективнее B-деревьев (B-Tree) для поиска по равенству в больших таблицах, занимают меньше места, но не поддерживают сортировку и диапазоны. 

GIN (Generalized Inverted Index — обобщенный инвертированный индекс) в PostgreSQL предназначен для индексации составных значений, таких как массивы, JSONB, полнотекстовый поиск (tsvector) и триграммы. Он хранит отсортированный список лексем (ключей), ссылающихся на идентификаторы строк, что обеспечивает быстрый поиск элементов внутри сложных структур. GIN строит B-дерево ключей, указывающих на список совпавших идентификаторов строк (posting list).

Индекс может быть по определенным строчкам (когда селект делаем с where) или по агрегации, чтобы еще раз не пересчитывать.

Очистка индексов в PostgreSQL с помощью команды VACUUM — это процесс удаления ссылок на удаленные или устаревшие строки (так называемые «мертвые кортежи» или bloat) из индексов таблицы.
...
Советы
- Не использовать в небольших таблицах
- Не использовать в таблицах с частыми массовыми update, insert
- Не использовать для столбцов, где есть много null
- Не индексировать столбцы, которые часто обрабатываются